...
- 9 种鱼使用 `FishArt.tsx` + `fishing.css` 的 CSS 轮廓、鳍、鳞片与动画，无生图或图片依赖。12 种恐龙/蛋/骨架定义为橡皮泥模型，收藏使用轻量轮廓，进入箱庭按需加载 3D 模型。保持旧 `speciesId` / `time-relic` 存档键兼容。详见 [恐龙箱庭](./DINOSAUR-GARDEN.md)。
...
`runVRSession` 的水域/市场分支各使用一次正常 LLM 调用。角色钓鱼在同一轮给出保留／放生、反应与可选私聊分享；首次图鉴播报由程序完成，交易留到布告板。市场分支选择购买、响应、回复、发帖、撤板或路过。市场私聊分享目前仍保存在活动卡，未单独发送普通聊天消息。
