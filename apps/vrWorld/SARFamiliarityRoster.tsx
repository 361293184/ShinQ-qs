...
import { ArrowLeft, BookOpen, CaretDown, LockKey, Play, Star, Users } from '@phosphor-icons/react';
...
export function SARFamiliarityRoster({ onBack, onOpenScene }: { onBack: () => void; onOpenScene: (npc: FamiliarityNpc, sceneId?: string) => void }) {
    ...
    useEffect(() => { root.current?.querySelector<HTMLButtonElement>('header button')?.focus({ preventScroll: true }); }, []);
    useEffect(() => {
    ...
    return <section className="sar-familiarity-roster" ref={root}>
        ...
        <main className="sar-roster-scroll" ref={scroll}>
            ...
            {error ? <p role="alert" className="sar-roster-error">{error}</p> : tab === 'profile' ? <article className="sar-roster-biography" key={npc}>
                ...
                <p className="sar-roster-hint">相遇过的话题会留在这里，随时可以再看一遍。</p>
