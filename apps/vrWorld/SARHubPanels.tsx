...
import { ArrowLeft, BookOpen, CaretDown, Coins, Cpu, Package, Stack, X } from '@phosphor-icons/react';
...
export function SARHubPanels({ panel, onClose, npcEnabled, onChangeNpc, userProfile, characters, backRef }: {
    ...
    useEffect(() => { setSelectedId(null); setFilter('all'); }, [owner.id]);
    useEffect(() => {
        ...
        void (async () => {
            try { await ensureSARCommerce(); await mutateFishingMarket(state => ensureActorAccounts(state, actors)); refresh(); }
            catch (cause) { if (live) setError(cause instanceof Error ? cause.message : '仓库暂时无法读取'); }
        })();
    ...
    useEffect(() => {
        ...
        const render = () => JSON.stringify({ mode: 'sar-hub', panel, npcEnabled, ownerId: owner.id, balance: market?.accounts[owner.id], filter, items: shown.map(({ id, title, count, status }) => ({ id, title, count, status })) });
    ...
    }, [panel, npcEnabled, owner.id, market, filter, collection]);
    ...
    return <div className="sar-hub-backdrop">
        <section ref={root} className="sar-hub-panel" role="dialog" aria-modal="true" aria-label={panel === 'settings' ? '活动室设置' : collection ? '收集图鉴' : '随身仓库'} onKeyDown={keyDown}>
            {collection && market ? <SARCollectionView market={market} owner={owner} actors={actors} onOwnerChange={setOwnerId} onClose={() => setCollection(false)} backRef={collectionBack}/> : <>
            ...
            {panel === 'settings' ? <main className="sar-hub-settings">
            ...
            </main> : <main className="sar-hub-warehouse">
                <div className="sar-hub-owner"><span>正在查看</span><label><select aria-label="仓库主人" value={owner.id} onChange={event => setOwnerId(event.target.value)}>{actors.map(actor => <option key={actor.id} value={actor.id}>{actor.id === 'user' ? `我 · ${actor.name}` : actor.name}</option>)}</select><CaretDown size={16}/></label></div>
                <KanataTitleEditor key={owner.id} ownerId={owner.id}/>
                {error ? <p role="alert" className="sar-hub-error">{error}</p> : !market ? <p role="status" className="sar-hub-muted">正在打开仓库…</p> : <>
                    ...
                    <nav className="sar-hub-filters" aria-label="物品分类">{([['all', '全部'], ['chip', '芯片'], ['module', '模块'], ['catch', '收藏']] as const).map(([value, label]) => <button type="button" key={value} aria-pressed={filter === value} onClick={() => { setFilter(value); setSelectedId(null); }}>{label}</button>)}</nav>
                    {shown.length ? <div className="sar-hub-items">{shown.map(item => <button type="button" className="sar-hub-item" key={item.id} aria-pressed={selectedId === item.id} onClick={() => setSelectedId(selectedId === item.id ? null : item.id)} aria-label={`${item.title} · ${item.count} 件 · ${item.status}`}>
                        <span className={`sar-hub-item-art is-${item.kind}`}>{item.speciesId ? <FishArt speciesId={item.speciesId} size={84}/> : item.kind === 'chip' ? <Stack size={35} weight="duotone"/> : <Cpu size={35} weight="duotone"/>}<b>×{item.count}</b></span><strong>{item.title}</strong><small>{item.status}</small>
                    ...
                    {selected && <aside className="sar-hub-item-detail" aria-live="polite"><button type="button" aria-label="收起物品详情" onClick={() => setSelectedId(null)}><X size={17}/></button><h3>{selected.title}</h3><p>{selected.detail}</p><small>{selected.count} 件 · {selected.status}</small></aside>}
