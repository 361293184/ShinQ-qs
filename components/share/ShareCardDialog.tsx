...
function ShareCardDialog({ options, card, onDone }: Props) {
    ...
    return <dialog ref={dialog} className="sully-share-dialog" aria-labelledby="sully-share-heading"
        onCancel={event => { event.preventDefault(); if (!working.current) onDone('cancelled'); }}>
        ...
        <div className="sully-share-body">
            ...
            <fieldset className="sully-share-fields" disabled={busy}>
                ...
                {tooLarge && <p className="sully-share-error">内容超过 64 MB，请使用原格式导出。</p>}
                {(error || renderError) && <p role="alert" className="sully-share-error">{error || renderError}</p>}
            </fieldset>
        </div>
