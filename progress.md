...
2026-09-12 鈥?WeChat bridge: scan-binds-character model
- Replaced contact-ID binding table with 'scan binds character': scanning in a character's settings page binds that WeChat bot to the character (1 WeChat = 1 character). Rebind is one click (POST /wx/bot/bind), no re-scan; contact IDs / CharLink / links_json / perChar all removed.
- Worker endpoints now: /wx/bot/qr, /wx/bot/qr/status?charId=, /wx/bot/bind, /wx/bot/check, /wx/bot/remove, /wx/config(llm only), /wx/pack, /wx/outbox(+ack), /wx/status. processIncoming routes by bot.charId.
- Redeployed with wrangler deploy --config wrangler.toml (root wrangler.jsonc hijack avoided); /health OK; cron * * * * * intact. tsc clean, build:workers + vite build pass.
- User still to do in-app: fill worker URL, save LLM creds (one-click from chat API), scan QR, upload character context.
2026-09-12 鈥?WeChat bridge: auto context sync
- Added utils/wechatBridge/auto-sync.ts: auto re-uploads character fire_pack when cloud has none (first-use auto-provision) or local chat is newer than cloud chatBuiltAt (+5s tolerance); WeChat-side chatting never triggers (worker appends and bumps chatBuiltAt itself); 5-min per-character throttle, in-flight mutex, all failures silent.
- Hooked autoRefreshContexts into global-poller (after drain, covering start/foreground/interval). drainWechatOutbox now merges with persistTimestamp=item.at so backfilled bubbles show real WeChat send time and offline catchup cannot be misread as app chatting.
- Manual upload button kept as fallback (card hint added); worker unchanged (chat_built_at guard already prevents stale overwrites).
- Verified: tsc clean, build:workers OK, vite build 32.6s. Deliberately deployed sully-os-site with the new build (old live UI was the contact-binding model, incompatible with the new worker API). /health of wechat-bridge unaffected.

2026-09-13 — WeChat bridge: cron heartbeat, false-expiry fix, config optimistic lock, reply format
- Root-caused the "cron never runs" symptom: the scheduled handler WAS firing but each run was killed before it could persist state (40s long-poll vs the shorter scheduled wall-clock budget). Cron poll timeout lowered to 20s; completion now ~15-19s. Added a wx_heartbeat row written at the very start of scheduled() so "did cron run / how far did it get" is always answerable (this is what proved the diagnosis).
- Fixed the false "登录已过期，需要重新扫码" that made the user re-scan for nothing: -14 (session expired, per protocol docs, also triggered by transient causes) is now retried once with a dropped cursor before being believed; expired is cleared on any successful poll; expired bots are no longer skipped forever (5-min low-frequency probe so a recovered session comes back by itself). Card and 「检查连接」 now target the bot bound to the current character instead of bots[0], and pass its botId.
- Removed the double-poller: the App-side fallback now reads /wx/status first and only issues getupdates when the cloud heartbeat is stale (>150s). The official protocol doc explicitly warns against concurrent getupdates (duplicate consumption / lost messages) — concurrency is what fed the server the -14 in the first place.
- Re-scan now auto-prunes: /wx/bot/qr/status (confirmed) and /wx/bot/bind both drop other bindings of the same character via pruneSiblingBots, so one WeChat keeps exactly one live session. Latest scan wins.
- Found and fixed a silent data-loss bug: the whole config is one JSON row and cron wrote it back every minute from a snapshot up to ~19s old, so user actions were rolled back (observed: /wx/bot/remove returned removed:1, the bot reappeared a minute later). saveConfig is now optimistic-locked on updated_at, poll paths merge per-bot state via applyBotState, and user actions go through mutateConfig (read-modify-write with retry).
- Reply shaping now matches SullyOS: <inner_voice> is stripped using the client's own utils/innerVoice (single source of truth), and bubbles split on explicit line breaks (same semantics as ChatParser.chunkText) instead of the never-emitted "---" separator; overflow beyond 8 bubbles merges into the last one instead of being dropped. Verified by compiling the real reply.ts with esbuild and running cases incl. the exact multi-line case from instant-push's own test.
- Deployed to sullyos-wechat-bridge (version cc6a02cf); heartbeat ok:18562ms, single binding retained after deletion, tsc clean on changed files, build:workers 46.0 KB.
- End-to-end verified with the App fully closed: three rounds of WeChat messages were received and answered by cron alone (pack 503 -> 509, todayMessages 2 -> 8), the whole run finishing in 19.1s so the scheduled budget has headroom; the ledger shows one multi-line reply split into five separate bubbles (06:37:43-48), i.e. the new line-based segmentation and inner_voice stripping work in production. Six increments queued in the outbox for the app to merge on next launch.


2026-09-13 — WeChat bridge: share-with-friends onboarding (walkthrough + init button)
- Decided with the user: no 代建, no multi-tenant, no merging the bridge into the amsg backend, no in-app one-click deploy. The deliverable is a walkthrough doc plus the one tiny change that makes it actually followable.
- Worker: added POST /wx/init (idempotent CREATE TABLE IF NOT EXISTS over the 5 tables) and storage{schemaReady,missingTables,tableCount} to /wx/status. /wx/status now short-circuits when the schema is missing instead of throwing "no such table" and 500ing, so the card can offer an init button rather than showing "can't connect".
- DDL now lives in two places on purpose (schema.sql for humans/CLI, src/schema.ts at runtime) with a drift-guard test comparing them statement-by-statement, plus an init-flow test that drives the real handler against a fake D1 for the empty-database path (empty -> readable status -> init -> ready -> idempotent).
- Frontend: the card shows a schema health row and an 「初始化数据表」 button inside the connection settings, plus a link to the walkthrough. sync.ts gained initWechatSchema(); types.ts gained WechatBridgeStorageInfo. MASTER_KEY turned out to be any string (deriveKey = SHA-256(masterKey + ':' + salt)), so the walkthrough can tell friends to type a long random string instead of installing openssl.
- Docs: new docs/wechat-bridge-setup-walkthrough.md (panel-first route, CLI appendix, five-step self-check, troubleshooting table, privacy/protocol boundaries, plus a forwardable blurb for friends); the bridge README points at it and its stale "token 过期只能重扫" line now describes the self-healing behaviour; self-deploy-workers.md points at it.
- Verified: 7 unit tests pass, tsc clean on the changed files, bridge bundle 46 -> 49 KB, production build passes, deployed (version ce1dae22); the live instance reports storage{schemaReady:true,tableCount:5}, cron heartbeat ok:~19s, binding and message counts unchanged, and /wx/init on the live DB is a confirmed no-op.
- Caveat: the card's walkthrough link and the walkthrough's bundle link point at the public repo, so both 404 until this commit is pushed. Panel button labels were written from the dashboard's usual wording; a friend may see slightly different labels.


2026-09-13 — Site auto-deploy + WeChat bridge actually visible on the live site
- Root cause of "I refreshed but there is no WeChat bridge": three wiring changes were never committed — apps/Character.tsx (renders <WechatBridgeCard char={formData} /> in the 设定 tab), components/PhoneShell.tsx (mounts <WechatBridgePoller />), scripts/build-workers.mjs (wechat-bridge build target). Committed as f6abb83d and pushed to main on both remotes. The other six "modified" files from git status were CRLF-only noise: `git diff --numstat` reports 0 changed lines for them.
- Second root cause, and the honest answer to "it always deployed fine before": there was never a working auto-deploy. deploy-cloudflare-worker.yml listens on master only (this repo has main), so it never ran; the only workflow that ran on every push to main was Deploy to GitHub Pages, which failed every single time (Pages is not enabled) — observed failures on 4a196838, f6abb83d and earlier. The live site had always been updated by a local build + `wrangler deploy` instead.
- Deployed the site by hand: `VITE_HIDE_BUILD_BADGE=1` (the local branch is sync-upstream, a non-release branch, so the badge would otherwise ship) → `pnpm run build` → `npx --yes wrangler@4.122.0 deploy` from the repo root (root wrangler.jsonc = sully-os-site, assets ./dist). New version 5700f70c is serving 100% of traffic; `wrangler versions list` shows the previous live build was 11be57e0 from 2026-09-12T16:26 — which is exactly why the app showed "有新版本" while containing no bridge.
- Verified the built assets carry the bridge: dist/version.json = f6abb83d; dist/assets/Character-*.js contains the 微信桥 card and its walkthrough link; dist/assets/index-*.js contains the poller; dist/wechat-bridge-worker.bundle.js is emitted. Caveat recorded: this sandbox cannot reach *.workers.dev (both curl and web fetch time out), so the live-page look is left to the user's phone and to the CI smoke test.
- Added .github/workflows/deploy-site-main.yml rather than editing upstream's two workflow files (qegj567-cloud maintains them, and touching them conflicts on every Merge upstream). It triggers on push to main + workflow_dispatch, mirrors the upstream steps (pnpm 10 / node 22 / --frozen-lockfile / assets-only guard / wrangler@4.122.0), runs only on the public repo via `if: github.repository == '361293184/ShinQ-qs'` (the same commit is pushed to both repos and two concurrent uploads would overwrite each other), fails loudly with a pointer to the docs when CLOUDFLARE_API_TOKEN is missing instead of silently skipping the deploy, and curls the live version.json afterwards to print whether what is online really is the build just made.
- Docs: docs/site-auto-deploy.md — create the Cloudflare token (Account → Workers Scripts → Edit), paste it into Settings → Secrets and variables → Actions → CLOUDFLARE_API_TOKEN, the optional Variables (CLOUDFLARE_ACCOUNT_ID / SITE_URL / UMAMI_*), how to run it, a failure table, and the manual-deploy fallback. The account id 70592357c9a42c223eae8375e1bddcb2 is baked in as a default (not a secret; upstream hardcodes its own the same way).
- First run of the new workflow (a8e86273, push, 2026-09-13T08:04:57Z, run 34746796427) proves the trigger now works and stops exactly at "Check deployment credentials" with everything after skipped, i.e. the designed explicit failure. Until the user adds the token, deploys stay manual.

2026-09-13 — Chat prompt: 「对话中的称呼规范」stops characters addressing the user by 姓+职称 or inventing nicknames
- Problem: characters kept calling the user 姓＋职称 (X老师 / X总 / X小姐) and improvising nicknames (being called 小管家婆 after reminding the character once), then fossilising them into verbal tics. She had said explicitly she disliked it and it came back after a while. She confirmed almost every character does it (so it is the model's own politeness habit, not a character card) and chose the minimal route: strengthen the prompt only, no data structures.
- Root cause, verified: the code never fed any of this in. The user block only injects the bare name (utils/context.ts 「互动对象 (User)」); grepping chatPrompts for 老师|小姐|先生|女士|称呼 gives zero hits; the only 老师 inside a prompt is a prohibition (songPrompts). Why the correction never sticks has four layers: the rolling context window pushes that sentence out, the memory system never extracts "communication preferences" (room-plate rules admit only basics / important others / life-shaping events, 门槛极高), the project deliberately writes no banned words (粉色大象 comments in chatPrompts:1146-1149 and context.ts:313-319), and there is no output-side cleanup to rewrite wording.
- Wording: she first sent a 9-clause 《forms_of_address》 that named examples (某总 / 某老师 / 小管家婆 / 小醋包 / 小财迷 / 小笨蛋 / 操心鬼). I flagged that naming them risks priming exactly those tokens, and she agreed to drop them — the shipped text keeps all 9 clauses but replaces the three example groups with category descriptions (姓氏＋职位 / 角色化的头衔 / 昵称 / 外号). Everything else is verbatim hers.
- Placement turned out to be the load-bearing decision: not beside the user-profile block, but at the very end of the stable core context, immediately after 「表达底线 (Anti-Filler)」. Reason: she had already tried the same text as a worldbook entry and it did nothing — worldbook lands in the 作者注释 reference section, which the model reads as background material rather than a behavioural constraint. It reuses its neighbour's guard (if (!groupOptions)), so multi-actor surfaces (group chat, game room, multi-actor story theatre) skip it and no duplicate copies are injected.
- Files: utils/context.ts (module-level FORMS_OF_ADDRESS(charName, userName) plus an 8-line injection, both exported for tests); utils/apiCallLog.ts (+1 line registering 对话中的称呼规范 in FIXED_PROMPT_LABEL_PREFIXES so the 输入构成 panel folds it into the 固定提示词 row — that panel doubles as a self-check that the rule really is in the prompt); utils/contextAddressRule.test.ts (new).
- The regression test locks six things: the key clauses are present; both placeholders are substituted (no ${charName} / ${userName} leak); the text contains none of 老师|小姐|先生|女士|老板|师傅|某总|管家婆|醋包|财迷|笨蛋|操心鬼; it stays in the stable side under deferVolatile and is absent from buildVolatileCoreState; it is not injected for multi-actor calls; and it is injected exactly once.
- Coverage audited (code-explorer) before touching anything: buildCoreContext has 40+ call sites covering private chat, calls, dates, phone checks, journal, study, songwriting, fanwai, relationships, XHS, bank, world home, active messages and the WeChat bridge (wechatBridge/pack.ts → buildSystemPrompt → buildCoreContext). Only multi-actor surfaces skip, and no second path that tells a character how to address the user exists.
- Full suite: 5304 tests, 5274 pass, 30 fail across 28 suites. All 30 are pre-existing source-shape "wiring/audit" tests that regex specific UI files (apps/Settings.tsx, apps/Chat.tsx, components/chat/MessageItem.tsx …) — none reads context.ts or apiCallLog.ts, every context/apiCallLog suite is green, and the 5 new tests pass. The working tree had no other uncommitted source edits at the time, so those failures were already failing at HEAD.
- Shipped: commit 530d4996, pushed to main on both remotes; rebuilt with VITE_HIDE_BUILD_BADGE=1 (the local branch sync-upstream is not a release branch, so the dev badge would otherwise ship) and deployed sully-os-site — version bbb57043-4c28-46c7-8326-852a88b4bd93 at 100% of traffic. The WeChat side re-bakes on its own once she chats with that character in the app (auto-sync fires when local is newer than cloud), or immediately via the card's manual upload button.
- Honest ceiling, stated to her twice: this is a prompt-level nudge, not a filter. Expect 姓＋职称 to drop sharply (it comes from the model padding with politeness when the relationship is undefined), improvised nicknames to drop moderately (they come from a creative impulse rather than a template), and nicknames she has accepted plus intimate character cards to keep some. Escalation ladder if it still shows a week later: (1) add a line to that character's 核心指令 (persona weight is highest), (2) make her correction a persistent preference, (3) output-side replacement (hardest, hurts naturalness).
- Deliberately not done: no settings toggle (it would need new UserProfile / CharacterProfile fields plus UI plus backup compatibility), and no touching memory extraction, context range, or output post-processing.

2026-09-13 — 微信桥改成多租户（一人部署、多人扫码即用，名额 10）
- 动机：原来微信桥是单租户（一份配置、一枚全局密钥、数据无"谁"维度），别人要用得自己部署一份；且同人手机+电脑连同一链接时微信消息会互相抢（outbox 认领即删）。
- 做法：前端首次打开时浏览器无感生成 128 位设备身份（localStorage，用户不需要填、不需要分发），请求带 X-Client-Token，服务端 owner = sha256(身份) 前 16 位；所有配置/pack/台账/增量/LLM 凭据按 owner 隔离（SQL 层强制，新增并发测试 guard）。名额 WX_MAX_OWNERS 默认 10（面板改数字即刻放宽，不用重部署）；增量队列改为 72 小时保留窗口 + 各设备按自己游标补收（修掉手机/电脑互抢的 bug）；轮询改按 owner 分组并行、并发跟随在线人数 + 总预算 25s 守卫 + 可选分片退路（WX_POLL_SHARD），cron 不再被平台掐死。
- 邀请口令（WX_JOIN_CODE）按用户要求已取消：默认就是"打开链接、扫码即用"。用户会把仓库设为私有，不靠口令挡人，靠身份隔离 + 名额兜底。
- 迁移：生产 D1 已线上补列（wx_packs/wx_messages/wx_outbox 加 owner，老数据回填 'main'），已先 d1 export 备份（.codebuddy/wechat-bridge-backup.sql）。新 worker 上线后 cron 心跳 ok:19s spaces:1 bots:1，干净跑通。
- 部署：worker=wechat-bridge/wrangler.toml（显式 --config 避免被根 wrangler.jsonc 劫持到站点）；站点=sully-os-site。备份 commit 3ccff214。
- 用户必做一步：打开微信桥卡片 → 会看到"检测到旧数据（N 个微信 / M 份上下文）"→ 点"并到这台设备"→ 原 main 空间里的微信绑定与上下文就认领到这台设备的身份下。另一台设备（手机或电脑）打开后，在那台卡片里点"粘贴"，把第一台显示的"这台设备的身份"粘过去，两台共用同一份数据。
- 隐私边界（给朋友说清楚）：朋友共用你的私服，他们的微信登录态（bot token 用你的 MASTER_KEY 加密存你的 D1）技术上你能解开——这是"朋友共用一个私服"的信任前提。某人的身份泄露只影响他自己，可在卡片"移除绑定"断开；名额空着几乎零成本。
- 测试：worker 21/21 + 前端设备身份 4/4 全绿；新增 multitenancy.test.ts（隔离/名额/认领/老密钥兼容）+ SQL 隔离守卫（三张按 owner 分区的表漏 owner 直接红）。
