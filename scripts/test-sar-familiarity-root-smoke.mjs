...
const idle=()=>page.waitForFunction(()=>{const state=JSON.parse(window.render_game_to_text?.()||'{}');return state.mode==='sar-familiarity'&&!state.busy&&!state.error&&!document.querySelector('.srf-script>p')?.textContent?.includes('正在走进活动室');});
...
try{
    await page.goto('http://127.0.0.1:5177/test/fixtures/kanata.html?npcs=show');await button('SAR').click();await button('与凯恩交谈').waitFor();
    ...
    await shot('01-caian-topic');await button('确实').click();await idle();assert.equal((await state()).node,'answer-1');await shot('02-caian-reply');
    await button('继续对话').click();await idle();assert.equal((await state()).finished,true);await shot('03-topic-collected');
    ...
    await button('离开对话').click();await button('与凯恩交谈').click();await idle();assert.equal((await state()).scene,undefined,'same day does not repeat the topic');await button('离开对话').click();
    ...
    await button('那我不说了').click();await idle();await button('继续对话').click();await idle();assert.equal((await state()).finished,true);await button('离开对话').click();
    ...
    await button('回顾彼方也太方便了吧').click();await idle();assert.equal((await state()).replay,true);await shot('06-replay');await button('你重点错了').click();await idle();await button('继续对话').click();await idle();
    assert.equal((await state()).finished,true);const after=await page.evaluate(()=>JSON.parse(localStorage.getItem('vr_fishing_market_v1')));
    ...
    await button('返回名册').click();await page.locator('.sar-familiarity-roster').waitFor();assert.equal(await page.locator('.srf-dialog').count(),0);
