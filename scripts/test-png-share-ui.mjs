...
try {
    ...
    for (const [label, style] of [['留白相纸', 'paper'], ['全幅海报', 'poster'], ['横版名片', 'business']]) {
        ...
        await page.screenshot({ path: `${out}/${style}-desktop.png` });
    ...
    await page.getByRole('button', { name: '导出 PNG 分享图' }).scrollIntoViewIfNeeded();
    ...
    writeFileSync(`${out}/result.json`, JSON.stringify({ passed: true, errors, checks: ['three layouts', 'image upload', 'pixel decoding', 'metadata and payload', '320px layout', 'long text', 'cancel and retry', 'original JSON', 'CSS round-trip and wrong kind', 'real character import'] }, null, 2));
