...
const node=async(id,nodeId,line=0)=>{
    ...
    await button(id.startsWith('C')?'与凯恩交谈':'与艾文交谈').click();await page.locator('.srf-dialog').waitFor();await page.waitForFunction(({id,nodeId})=>{const s=JSON.parse(window.render_game_to_text());return s.mode==='sar-familiarity'&&s.scene===id&&(!nodeId||s.node===nodeId)&&!s.busy;},{id,nodeId});
};
const next=async()=>{await button('继续对话').click();await page.waitForFunction(()=>!JSON.parse(window.render_game_to_text()).busy);};
try{
    ...
    await page.evaluate(async()=>{
        ...
        await DB.saveUserProfile({...await DB.getUserProfile(),name:'小雨',vrState:{enabled:true,currentRoom:'sar',chibi:{img}}});
    ...
    await close();await button('与艾文交谈').click();await page.waitForFunction(()=>JSON.parse(window.render_game_to_text()).node==='working');assert.equal(await page.locator('.srf-stage .sar-dialogue-cast__actor').count(),2,'resume preserves current guest speaker');
    await next();assert.equal(await page.locator('.srf-stage .cast-caian').count(),0,'returning to the protagonist restores the personal view');await shot('23-back-to-personal');
    await node('C1-SPECIAL','member-card');await button('就用这个形象').waitFor();assert(await button('继续对话').isDisabled());await shot('03-membership');await button('就用这个形象').click();await next();
    ...
    await button('拍好了').click();await shot('05-photo-saved');await next();
    ...
    const saved=await page.evaluate(()=>localStorage.getItem('vr_fishing_market_v1'));assert.equal(await page.locator('.srf-stage .sar-dialogue-cast__actor').count(),1,'replay uses the same solo rule');await shot('15-replay');await button('确实').click();await next();assert.equal(await page.evaluate(()=>localStorage.getItem('vr_fishing_market_v1')),saved);await close();
