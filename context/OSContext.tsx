...
export const OSProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const exportSystem = async (mode: 'text_only' | 'media_only' | 'full'): Promise<Blob> => {
      try {
          ...
          const allStores = [
              ...
              'room_notes', 'groups', 'journal_stickers', 'social_posts', 'courses', 'games', 'worldbooks', 'story_theaters', 'story_theater_presets', 'story_theater_masks', 'novels', 'fanwai_stories', 'songs',
  ...
  const importSystem = async (fileOrJson: File | string, options: { source?: 'sully' | 'csy' } = {}): Promise<void> => {
  ...
  // 预览 CSY-OS 备份：读取并检查是否合法，返回迁移报告摘要（角色数/记忆数等）。
