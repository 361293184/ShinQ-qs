...
export interface BuildChatPayloadInput {
    ...
    /**
     * 「角色主动发照片」控制。开启时 buildSystemPromptParts 会注入「主动发图」规则段，
     * 让角色在合适的场景自发生图。默认不传 / false=关闭。
     * - enabled: 开启开关（用户去 Settings → 生图 → "允许角色主动发照片"打卡后启用）
     * - rate: 频率档位（保守/适中/大胆），影响规则段里给的"每 N 轮 1 张"指引
     * 不传则规则段不注入。
     */
