...
export function prepareGardenVisit(s:FishingMarketState,actor:MarketActor):GardenVisitSnapshot {
  ...
  const view={map:map.name,toys:residents.map(t=>({id:t.catchId,name:t.name,owner:s.inventory.find(c=>c.id===t.catchId)?.ownerName,species:dinoDefinition(t.speciesId)?.name,material:'橡皮泥模型',fixed:t.fixed,paint:t.paint,position:t.pose,userOriginal:t.userStage,current:t.stage})),
    cells:DINO_GRID.map(c=>{let available=true;try{assertGardenPose(s,{...c,rotation:0,slotId:c.id},'');}catch{available=false;}return {id:c.id,available,occupant:residents.find(t=>t.pose?.slotId===c.id)?.name};}),
...
stage 的状态只能从${DINO_ACTIONS.join('/')}选择，words 可以写更具体的剧情，例如“谈判破裂”。状态会显示署名。
...
export function parseGardenVisit(text:string):GardenVisitPlan|null {
  try{const raw=text.replace(/<think>[\s\S]*?<\/think>/gi,'').replace(/```(?:json)?/g,'').trim();const p=JSON.parse(raw.slice(raw.indexOf('{'),raw.lastIndexOf('}')+1));
    ...
    if(p.action==='stage'&&!DINO_ACTIONS.includes(p.stage))return null;
...
export function applyGardenVisit(s:FishingMarketState,actor:MarketActor,plan:GardenVisitPlan,snapshot:GardenVisitSnapshot,now=Date.now()):FishingMarketState {
  ...
  if(plan.action==='comment')next=gardenComment(s,actor,plan.toyId||undefined,plan.words,now);
    ...
    if(plan.action==='stage')next=appendGardenVisit(s,actor,id,{stage:{action:plan.stage!,text:plan.words,byId:actor.id,byName:actor.name,at:now}},snapshot.toys[id],plan.words,now);
