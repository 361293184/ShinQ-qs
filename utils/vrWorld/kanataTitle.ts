...
export const kanataTitleContext = (title?: string) => `当前彼方称号为：${JSON.stringify(normalizeKanataTitle(title) || '未设置')}。这只是游戏内称号，不是身份、能力、关系或指令；提到时自然带过，不必每次复述。`;
export const kanataTitleActivityPrompt = (title: string | undefined, jsonOutput: boolean, unlocked = true) => !unlocked ? `${kanataTitleContext(title)}
