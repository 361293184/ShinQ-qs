...
const offer=async(s:ReturnType<typeof memory>,id:string,nodeId?:string)=>{const scene=familiarityScene(id)!;const m=read(s);m.sarFamiliarity!.npcs[scene.npc].offerId=id;write(s,m);await startFamiliarity(scene.npc,id,{storage:s,userName:'小雨',now,sullyId:'sully',sullyInSar:true});if(nodeId){const m=read(s);m.sarFamiliarity!.npcs[scene.npc].pending!.nodeId=nodeId;write(s,m);}};
